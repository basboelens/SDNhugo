...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        ...
        # DHCP for ALL BUILDINGS (3 consolidated VLANs) 
        dhcp10 = self.addHost('dhcp10', ip='10.0.10.10/24', defaultRoute='via 10.0.10.254') # Office
        dhcp20 = self.addHost('dhcp20', ip='10.0.20.10/24', defaultRoute='via 10.0.20.254') # Guest
        dhcp30 = self.addHost('dhcp30', ip='10.0.30.10/24', defaultRoute='via 10.0.30.254') # Management
...
# ------ Switch - Switch ----------------------------------------------------------------------------------- #
        ...
        # DHCP A (Connected to sAC for all three VLANs)
...
def run():
    ...
    DNS_SERVER_V6 = "2001:4860:4860::8888" # Standard Google Public DNS
    ...
    # ISP Route: Route the internal GUA space back to the NAT node
    ...
    nat_node.cmd('sysctl -w net.ipv6.conf.all.forwarding=1')
    ...
    #     host.cmd(f'sysctl -w net.ipv6.neigh.{host_intf}.base_reachable_time_ms=600000') 
    #     host.cmd(f'sysctl -w net.ipv6.neigh.{host_intf}.gc_stale_time=600') 

    # IP Forwarding in op de DHCP-hosts 
    # for dhcp_node in [dhcp10, dhcp20, dhcp30]:
    ...
    dhcp10.cmd('ip addr add 2042:100::254/64 dev dhcp10-eth1') # Office VIP
    
    dhcp20.cmd('ip addr add 10.0.20.254/24 dev dhcp20-eth1')
    dhcp20.cmd('ip addr add 2042:200::10/64 dev dhcp20-eth1')
    dhcp20.cmd('ip addr add 2042:200::254/64 dev dhcp20-eth1') # Guest VIP
    
    dhcp30.cmd('ip addr add 10.0.30.254/24 dev dhcp30-eth1')
    dhcp30.cmd(f'ip addr add 2042:300::10/64 dev dhcp30-eth1')
    dhcp30.cmd(f'ip addr add 2042:300::254/64 dev dhcp30-eth1') # Management VIP
    
    # Start dual-stack dnsmasq servers
    dhcp10.cmd(f"dnsmasq --interface={defaultIntf(dhcp10)} --bind-interfaces --dhcp-range=10.0.10.11,10.0.10.250,12h --dhcp-range=2042:100::11,2042:100::fffe,64,12h --dhcp-option=3,10.0.10.254 --dhcp-option=6,8.8.8.8 --dhcp-option=option6:dns-server,[{DNS_SERVER_V6}] --dhcp-sequential-ip --no-daemon &")
    dhcp20.cmd(f"dnsmasq --interface={defaultIntf(dhcp20)} --bind-interfaces --dhcp-range=10.0.20.11,10.0.20.250,12h --dhcp-range=2042:200::11,2042:200::fffe,64,12h --dhcp-option=3,10.0.20.254 --dhcp-option=6,8.8.8.8 --dhcp-option=option6:dns-server,[{DNS_SERVER_V6}] --dhcp-sequential-ip --no-daemon &")
    dhcp30.cmd(f"dnsmasq --interface={defaultIntf(dhcp30)} --bind-interfaces --dhcp-range=10.0.30.11,10.0.30.250,12h --dhcp-range=2042:300::11,2042:300::fffe,64,12h --dhcp-option=3,10.0.30.254 --dhcp-option=6,8.8.8.8 --dhcp-option=option6:dns-server,[{DNS_SERVER_V6}] --dhcp-sequential-ip --no-daemon &")
    
    # --- DNS64 CONFIGURATION ON NAT NODE (via dnsmasq) --- # REMOVED
    # nat_node.cmd(
    #     f"dnsmasq --port=53 --interface={natA_lan} --bind-interfaces "
    #     f"--enable-nat64={NAT64_PREFIX},{nat_wan} --listen-address={NAT_A_V6} "
    #     f"--server=2001:4860:4860::8888 --server=8.8.8.8 --no-daemon &" 
    # )
    ...
    # FIX: Correct IPv6 address prefix and add default route

    info('*** Applying IPv6 default route, correcting prefix, and priming NDP cache ***\n')
    
    # Correct v6 subnet from /128 to /64
    for name in HOST_NAMES:
        host = net.get(name)
        ...
        for line in ip_addr_output.splitlines():
            line = line.strip()
            # Search for any global address ending in /128
            if 'inet6' in line and 'scope global' in line: 
                parts = line.split()
                try:
                    # Find the part that contains the address/prefix
                    ip_part = next(p for p in parts if '/' in p)
                    if ip_part.endswith('/128'):
                         target_ip_full = ip_part
                         break
                except StopIteration:
                    continue

        if target_ip_full:
            new_ip_raw = target_ip_full.split('/')[0]
            new_ip_64 = f'{new_ip_raw}/64'

            # Force delete the incorrect /128 address
            host.cmd(f'ip -6 addr del {target_ip_full} dev {intf_name} 2>/dev/null || true')
            
            # Force add the correct /64 address
            host.cmd(f'ip -6 addr add {new_ip_64} dev {intf_name}')
            info(f'{name}: Corrected prefix: {target_ip_full} -> {new_ip_64}\n')
        else:
            info(f'{name}: IPv6 address prefix OK or already fixed.\n')

        # Configure default route and clear RA routes
        ...
        # # --- STEP 3: NDP Learning Fix (Priming the VIP) ---
        # host.cmd(f'ping6 -c 1 {router_ip} > /dev/null 2>&1')
    
    # # --- AGGRESSIVE NDP/L3 FLOW CACHE PRIMING FOR INTER-VLAN ROUTING ---
    # info('*** Aggressively priming NDP cache and L3 flows across all switches... ***\n')

    # VIP_LIST = ['2042:100::254', '2042:200::254', '2042:300::254']
    # dhcp30_node = net.get('dhcp30')
    
    # # 1. Priming L3 routes from the Management VLAN (most stable source)
    # for vip in VIP_LIST:
    #     # Ping all VIPs from Mgmt (including itself)
    #     dhcp30_node.cmd(f'ping6 -c 3 -i 0.1 {vip} > /dev/null 2>&1')

    # # 2. Priming the vital Mgmt -> NAT path
    # dhcp30_node.cmd(f'ping6 -c 3 -i 0.1 {NAT_A_V6} > /dev/null 2>&1')
    
    # # 3. Aggressive priming of L3 flows on the access switches by pinging the gateway
    # # This addresses the recurring "Destination unreachable: No route" issue on access switches.
    # for name in HOST_NAMES:
    #     host = net.get(name)
    #     # Force host to trigger L3 flow to the gateway
    #     host.cmd(f'ping6 -c 1 {NAT_A_V6} > /dev/null 2>&1') 

    # info('*** NDP/L3 Priming complete. ***\n')
    # time.sleep(15) # Give Faucet a final moment to install flows
    
    CLI(net)

    # Cleanup dnsmasq processes
    for host in [dhcp10, dhcp20, dhcp30]:
