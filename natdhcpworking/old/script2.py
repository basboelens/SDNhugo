...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
        ...
        natB= self.addNode('natB', cls=NAT, ip='10.0.30.2/24', defaultRoute='via 10.0.30.254')
...
# ------- Switch A1 --------------------------------------------------------------------------------------- #
        ...
        # DHCP
        self.addLink(switchA1, dhcp10, port1=100, port2=1)
        self.addLink(switchA1, dhcp20, port1=200, port2=1)
        self.addLink(switchA1, dhcp30, port1=300, port2=1)

        self.addLink(switchA1, switchB1, port1=23, port2=24)
        self.addLink(switchA1, natA, port1=24, port2=1)
        self.addLink(switchA1, switchA2, port1=25, port2=24)
...
# ------- Switch B1 --------------------------------------------------------------------------------------- #
        ...
        self.addLink(switchB1, natB, port1=20, port2=1)
        self.addLink(switchB1, switchB3, port1=21, port2=24)
        self.addLink(switchB1, switchB2, port1=22, port2=24)
...
def run():
    ...
    natA_lan = intf_to_peer(natA_node, 'sA1')
    natA_wan = intf_to_peer(natA_node, 'isp')
    natB_lan = intf_to_peer(natB_node, 'sB1')
    natB_wan = intf_to_peer(natB_node, 'isp')
    isp_to_natA = intf_to_peer(isp_node, 'natA')
    isp_to_natB = intf_to_peer(isp_node, 'natB')

    # ISP addresses
    isp_node.cmd(f'ip addr flush dev {isp_to_natA}; ip addr add 221.1.1.1/28 dev {isp_to_natA}')
    isp_node.cmd(f'ip addr flush dev {isp_to_natB}; ip addr add 221.1.1.2/28 dev {isp_to_natB}')

    # NAT configuration
    natA_node.cmd(f'ip addr replace 10.0.30.1/24 dev {natA_lan}')
    natA_node.cmd(f'ip addr replace 221.1.1.3/28 dev {natA_wan}')
    ...
    natB_node.cmd(f'ip addr replace 10.0.30.2/24 dev {natB_lan}')
    natB_node.cmd(f'ip addr replace 221.1.1.4/28 dev {natB_wan}')
    natB_node.cmd(f'ip route replace default via 221.1.1.2 dev {natB_wan}')
    ...
    dhcp10.cmd('dnsmasq --interface=dhcp10-eth1 --bind-interfaces --dhcp-range=10.0.10.11,10.0.10.200,12h --dhcp-option=3,10.0.10.254 --dhcp-option=6,8.8.8.8 --no-daemon --dhcp-leasefile=/tmp/dnsmasq-dhcp30.leases &')
    dhcp20.cmd('dnsmasq --interface=dhcp20-eth1 --bind-interfaces --dhcp-range=10.0.20.11,10.0.20.200,12h --dhcp-option=3,10.0.20.254 --dhcp-option=6,8.8.8.8 --no-daemon --dhcp-leasefile=/tmp/dnsmasq-dhcp30.leases &')
